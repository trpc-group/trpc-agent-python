...
The field is typed as the openai SDK's `ResponseCreateParams` (`openai.types.responses`), so IDEs
auto-complete every native parameter. All values are passed through verbatim to `responses.create`
without any SDK-side mapping. To control reasoning depth, pass `reasoning.effort` directly:
...
Supported effort values (`none`, `minimal`, `low`, `medium`, `high`, `xhigh`, `max`) vary by model —
