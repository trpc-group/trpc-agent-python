...
```python
from trpc_agent.agents import LlmAgent
from trpc_agent.agents import TimelineFilterMode
...
```python
from trpc_agent.agents import LlmAgent
from trpc_agent.agents import BranchFilterMode
...
构造用户历史记录：
```python
from trpc_agent.sessions import HistoryRecord
...
```python
from trpc_agent.models.tool_prompt import XmlToolPrompt
OpenAIModel(
    model_name="deepseek-chat",
    base_url="https://api.deepseek.com/v1",
    api_key=os.environ.get("API_KEY", ""),
...
实现细节可以参考 [XmlToolPrompt](../../../trpc_agent/models/tool_prompt/_xml.py)。

```python
from from trpc_agent.models.tool_prompt import ToolPrompt
...
```python
from trpc_agent.planners import PlanReActPlanner
...
from trpc_agent.tools import FunctionTool
from trpc_agent.types import GenerateContentConfig
from trpc_agent.planners import BuiltInPlanner
from trpc_agent.types import ThinkingConfig
...
```python
from trpc_agent.agents import LlmAgent
from trpc_agent.models import OpenAIModel, LLMModel
from trpc_agent.configs import RunConfig
...
LlmAgent也支持结构化输入(input_schema)，一般需要配合 [AgentTool](./tools.md) 使用，AgentTool会自动校验Agent的输入/输出是否符合schema，如下所示：
