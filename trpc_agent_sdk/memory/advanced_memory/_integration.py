...
def _setup_preload_memory_tool(
    ...
    preloader = AdvancedMemoryPreloader(
        memory_runtime,
        AdvancedModelMemoryRelevanceSelector(model),
    )
...
def setup_long_term_memory(
    ...
    *,
    ...
    tools = (_setup_long_term_memory_tools(agent, memory_runtime)
             if install_tools and memory_runtime.config.enabled else None)
