...
class _SqlStore:
    ...
    def __init__(self, config: AdvancedCompactConfig, paths: AdvancedMemoryPaths, storage: SqlStorage) -> None:
    ...
    async def _refresh_session_scope(self, db: Any, session_id: str) -> None:
        ...
        if expiry is None:
            return
        tables = ((SqlToolResult, (self._app_name, self._user_id, session_id)), )
...
class SqlTranscriptStore(_SqlStore):
    ...
    def _validate_record(record: Mapping[str, Any]) -> None:
        """Reject Event and Session Memory duplication in SQL."""
        if record.get("kind") in {"event", "session-memory-checkpoint"}:
            raise ValueError("SQL transcripts only store context-compression records")
...
class SqlAdvancedMemoryCleanup:
    ...
    def __init__(self, config: AdvancedCompactConfig, storage: SqlStorage) -> None:
        self._config = config
        self._storage = storage
