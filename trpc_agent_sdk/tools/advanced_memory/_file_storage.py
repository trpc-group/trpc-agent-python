...
"""Long-term memory storage owned by AdvancedMemory."""
...
def parse_memory_index(index: str) -> list[AdvancedMemoryIndexEntry]:
    """Parse standard entries from a MEMORY.md index."""
    entries: list[AdvancedMemoryIndexEntry] = []
    for line in index.splitlines():
        match = _MEMORY_INDEX_PATTERN.match(line.strip())
        if match is not None:
            entries.append(AdvancedMemoryIndexEntry(**match.groupdict()))
    return entries
