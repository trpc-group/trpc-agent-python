...
def _frontmatter(content: str) -> dict[str, str]:
    """Parse the simple single-line frontmatter used by AdvancedMemoryDocument."""
...
def _candidate_from_content(filename: str, content: str) -> AdvancedMemoryCandidate:
    """Build candidate metadata from a topic document."""
    metadata = _frontmatter(content)
    return AdvancedMemoryCandidate(
        filename=filename,
...
class AdvancedModelMemoryRelevanceSelector(AdvancedMemoryRelevanceSelector):
    ...
    @staticmethod
    def _build_prompt(query: str, candidates: list[AdvancedMemoryCandidate], limit: int) -> str:
