...
class AdvancedMemorySqlIndex(AdvancedMemorySqlBase):
...
class AdvancedMemorySqlTopic(AdvancedMemorySqlBase):
...
class SqlLongTermMemoryStore(_SqlStore):
    ...
    async def initialize(self) -> None:
        ...
        async with self._storage.create_db_session() as db:
            key = SqlKey(key=(self._app_name, self._user_id), storage_cls=AdvancedMemorySqlIndex)
            row = await self._storage.get(db, key)
            if row is None:
                await self._storage.add(
                    db,
                    AdvancedMemorySqlIndex(
    ...
    async def write_index(self, entries: list[AdvancedMemoryIndexEntry]) -> None:
        ...
        async with self._storage.create_db_session() as db:
            ...
            row = await self._storage.get(db, key)
            if row is None:
                row = AdvancedMemorySqlIndex(app_name=self._app_name, user_id=self._user_id)
    ...
    async def read_topic(self, topic_name: str) -> str | None:
        async with self._storage.create_db_session() as db:
            row = await self._storage.get(
                db,
                SqlKey(key=self._topic_key(topic_name), storage_cls=AdvancedMemorySqlTopic),
            )
            if row is None or self._expired(row.expires_at):
                return None
            await self._refresh_memory_scope(db)
    ...
    async def write_topic(self, topic_name: str, document: AdvancedMemoryDocument) -> Path:
        ...
        async with self._storage.create_db_session() as db:
            ...
            row = await self._storage.get(db, SqlKey(key=key, storage_cls=AdvancedMemorySqlTopic))
            if row is None:
                row = AdvancedMemorySqlTopic(app_name=key[0], user_id=key[1], topic_name=key[2])
                await self._storage.add(db, row)
    ...
    async def cleanup_once(self) -> None:
        ...
        async with self._storage.create_db_session() as db:
            ...
            indexes = await self._storage.query(
                db,
                SqlKey(key=tuple(), storage_cls=AdvancedMemorySqlIndex),
            )
            for index in indexes:
                topics = await self._storage.query(
                    ...
                    SqlKey(
                        ...
                        storage_cls=AdvancedMemorySqlTopic,
                    ),
                    SqlCondition(filters=[
                        AdvancedMemorySqlTopic.app_name == index.app_name,
                        AdvancedMemorySqlTopic.user_id == index.user_id,
                        AdvancedMemorySqlTopic.expires_at.is_(None)
                        | (AdvancedMemorySqlTopic.expires_at > now),
                    ]),
                )
