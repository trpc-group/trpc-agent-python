...
class LlmAgent(BaseAgent):
    ...
    def _should_enable_agent_transfer(self) -> bool:
        ...
        if (
            not self.disallow_transfer_to_peers
            and self.parent_agent
            and self._is_llm_agent(self.parent_agent)
            and len(self.parent_agent.sub_agents) > 1
        ):  # Has siblings
            return True
                ...
                if collected_tool_calls:
                    ...
                    try:
                        ...
                        if long_running_tool_ids and getattr(self, "parallel_tool_calls", False):
                            logger.warning(
                                "Long-running tools are mixed with parallel_tool_calls=True. "
