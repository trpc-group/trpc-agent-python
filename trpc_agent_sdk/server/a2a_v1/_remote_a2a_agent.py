...
# Same shape as models._httpx_client._DEFAULT_HTTP_CLIENT_TIMEOUT: connect
# fails fast; read/write/pool share a 10-minute hang ceiling so streaming can
# idle between SSE frames without blocking forever if the peer never responds.
_DEFAULT_HTTPX_TIMEOUT = httpx.Timeout(timeout=600.0, connect=5.0)

# Agent-card GET is a small JSON fetch and has no cancel racing; do not inherit
# the streaming read window.
_CARD_DISCOVERY_TIMEOUT = httpx.Timeout(connect=5.0, read=30.0, write=30.0, pool=5.0)


async def _aclose_stream_iterator(stream_iter: Any) -> None:
    """Close a remote SSE iterator without masking the caller's exception."""
    ...
    try:
        ...
        logger.debug("Failed to close remote A2A stream iterator", exc_info=True)
...
class TrpcRemoteA2aAgent(BaseAgent):
        ...
        try:
            if self._a2a_client is None:
                if self._httpx_client is None:
                    self._httpx_client = httpx.AsyncClient(timeout=_DEFAULT_HTTPX_TIMEOUT)
            ...
            if not self._initialized:
                await self._aclose_owned_httpx_client()

    async def _aclose_owned_httpx_client(self) -> None:
        """Close the httpx client created by this agent, if any."""
        ...
        try:
            ...
            logger.debug("Failed to close remote A2A httpx client", exc_info=True)
    ...
    async def _discover_card(self) -> AgentCard:
        ...
        return await resolver.get_agent_card(http_kwargs={"timeout": _CARD_DISCOVERY_TIMEOUT})
    ...
    async def _stream_with_cancel_check(
        ...
        try:
            ...
            await _aclose_stream_iterator(stream_iter)
