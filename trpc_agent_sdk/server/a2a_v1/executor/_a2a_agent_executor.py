...
class TrpcA2aAgentExecutor(AgentExecutor):
    ...
    def _get_user_session_from_task_metadata(
        ...
        """Extract (app_name, user_id, session_id) from the stored Task.

        ``execute()`` does not put these fields on the initial ``Task`` event.
        They are published on the working ``TaskStatusUpdateEvent``; a2a-sdk
        ``TaskManager`` merges that event metadata onto the persisted Task
        (``task.metadata.MergeFrom(event.metadata)``), which cancel then reads.
        """
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue):
        ...
        try:
            ...
            try:
                ...
                await self._enqueue_failure_event(
                    event_queue,
                    context,
                    str(ex),
                    metadata=metadata,
    ...
    async def _enqueue_failure_event(
        self,
        event_queue: EventQueue,
        context: RequestContext,
        message_text: str,
        *,
        aggregator: Optional[TaskResultAggregator] = None,
        metadata: Optional[dict[str, Any]] = None,
    ) -> None:
        """Publish a terminal failed status, optionally recording it on the aggregator.

        a2a-sdk 1.x task-mode streaming forbids a bare ``Message`` after the
        initial ``Task``; the whole failed-status event is enqueued instead.
        """
        failure = create_exception_status_event(
            task_id=context.task_id,
            context_id=context.context_id,
        ...
        if aggregator is not None:
            aggregator.process_event(failure)
        try:
            await event_queue.enqueue_event(failure)
    ...
    async def _handle_request(self, context: RequestContext, event_queue: EventQueue):
        ...
        # emit the working status (session keys live on event metadata).  The SDK
        # TaskManager merges that metadata onto the stored Task so cancel can
        # recover the run-period user_id/session_id.
        await event_queue.enqueue_event(
        ...
        try:
            ...
            await self._enqueue_failure_event(
                ...
                metadata={
                    **ex.get_custom_metadata(), "error_code": ex.error_code
                },
            )
            return
        except Exception as ex:  # pylint: disable=broad-except
        ...
        # convert_event_to_a2a_events already enqueues TASK_STATE_FAILED when the
        # runner yields an Event with error_code.  Re-emitting it here would send
        # a second terminal failed update; a2a-sdk 1.x DefaultRequestHandler does
        # not dedupe those.  AUTH_REQUIRED / INPUT_REQUIRED still need this
        # trailing status because conversion only notifies the aggregator.
        if aggregator.task_state == TaskState.TASK_STATE_FAILED:
            return
