...
class Runner:
    ...
    async def run_async(
        ...
        *,
        ...
        Raises:
            RunLimitException: If an agent exceeds one of its configured run
                limits.
        """
        ...
        with tracer.start_as_current_span("invocation") as invocation_span:
            ...
            try:
                ...
                trace_error_type = ex.error_code
                trace_error_message = str(ex)
                logger.warning("Run for session %s exceeded a configured limit: %s", session_id, ex)
                await cancel.cleanup_incomplete_function_calls(
                    session,
                    invocation_id=invocation_context.invocation_id,
                )
