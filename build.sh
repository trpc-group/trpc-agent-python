...
Environment behavior:
  - Reuses a healthy project .venv first and runs clean.sh against it.
  - Without .venv, reuses an active environment or creates .venv.
...
elif is_virtualenv_python .venv/bin/python; then
...
case "${INSTALLER}" in
    pip)
        if [[ "${CREATED_VENV}" == "1" ]]; then
            ...
            echo "Activate it in your shell with: source .venv/bin/activate"
            echo "For example:"
