...
def _dump_part(part: Part) -> None:
    ...
    if getattr(part, "thought", False):
        print(f"    [thought] <reasoning token>")
    ...
    async for resp in model.generate_async(request, stream=stream):
        ...
        if resp.usage_metadata:
            print(f"    [usage] thoughts={resp.usage_metadata.thoughts_token_count} "
                  f"prompt={resp.usage_metadata.prompt_token_count} candidates={resp.usage_metadata.candidates_token_count}")
...
async def test_tool_roundtrip(stream: bool) -> None:
    ...
    for fc in calls:
        ...
        parts.append(Part(function_response=FunctionResponse(
            id=fc_id, name=fc.name, response={"city": "北京", "weather": "晴", "temperature": 26})))
    round2 = LlmRequest(
        contents=[
...
async def test_streaming_tool_roundtrip() -> None:
    ...
    for fc in final_calls:
        ...
        parts.append(fcall_part)
        parts.append(Part(function_response=FunctionResponse(
            id=fc_id, name=fc.name, response={"city": "深圳", "weather": "多云", "temperature": 30})))
    round2 = LlmRequest(
