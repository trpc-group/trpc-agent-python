...
目录结构与框架现有 Skill 示例保持一致：示例根目录保留 `README.md`、`run_agent.py`、`run_review.py` 和 `agent/`，可复用 Skill 资产集中在 `skills/code-review/` 下，其中 `SKILL.md` 描述能力，`docs/` 放规则和沙箱策略文档，`scripts/` 放沙箱内执行脚本，`rules.json` 与 `filter_policy.json` 分别提供规则配置和 Filter policy-as-code。`fixtures/`、`sample_outputs/` 和 `evaluate_fixtures.py` 只服务本示例的回归验证，不混入 Skill 目录。
