...
def knowledge_search(query: str, top_k: int = 3) -> Dict[str, Any]:
    ...
    mock_docs = [
        {"title": "公司产品", "content": "公司有产品A（企业协作）、产品B（API/SDK）、产品C（个人笔记与待办）。"},
        {"title": "产品A", "content": "产品A 适用于企业协作，支持文档与任务管理。"},
        {"title": "产品B", "content": "产品B 提供 API 与 SDK，便于集成。"},
        {"title": "产品C", "content": "产品C 面向个人用户，提供笔记与待办。"},
    ]
    recall_docs = []
    for index, mock_docs_index in enumerate(mock_docs):
        # 字面召回
        if mock_docs_index['title'] == query:
            recall_docs.append(mock_docs_index)
    # 返回前 topk 的召回文档
    return {
        "query": query,
        "results": recall_docs[:top_k],
    }
...
def create_agent() -> LlmAgent:
    ...
    return LlmAgent(
        ...
        model=_create_model(),
        instruction=(
            "你是知识问答助手。用户提问时先调用 knowledge_search 检索相关知识，"
