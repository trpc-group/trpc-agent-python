...
Uses Phase 2 attribution results to target specific sections
(system_prompt / skill_prompt) of TargetPrompt and generates
optimized prompt candidates.

Two modes:
- fake: hard-coded optimization hints (no LLM API needed)
- real: delegates to trpc_agent.optimization.AgentOptimizer API

Two priority strategies:
- failure_driven: optimize the highest-frequency failure category first
- iterative: continue optimizing up to max_iterations rounds
"""
