...
async def test_backend_cannot_mutate_canonical_baseline_or_candidate_bundles(tmp_path: Path):
    ...
    prompt_calls = [(call[1], call[2], call[3]["system_prompt"]) for call in backend.calls if call[0] == "evaluate"]
    assert prompt_calls == [
    ...
    assert report.audit["candidate_prompts"]["candidate_a"] == {"system_prompt": "safe prompt"}
    ...
    artifact_prompt = Path(request.output_dir).joinpath(
        "runs",
        request.run_id,
        "candidate_prompts",
        candidate_artifact,
...
async def test_successful_backend_source_mutation_is_fatal(
    ...
    assert Path(request.target_prompt_paths["system_prompt"]).read_text(encoding="utf-8") == "backend source mutation"
...
@pytest.mark.parametrize(
    "failure",
    [RuntimeError("backend error after mutation"),
...
async def test_candidate_evaluation_error_rejects_candidate_and_continues(
    ...
    candidate_calls = [(call[1], call[2]) for call in backend.calls if call[0] == "evaluate" and call[1] != "baseline"]
...
@pytest.mark.asyncio
async def test_candidate_evaluation_error_makes_later_budget_gate_fail_closed(tmp_path: Path, ):
    request = _request(
        tmp_path,
        update_source=True,
        gate_config={
...
@pytest.mark.asyncio
async def test_all_candidate_evaluation_errors_finalize_rejected_audit_without_writeback(tmp_path: Path, ):
    request = _request(tmp_path, update_source=True)
    delegate = _safe_backend(candidate_count=2)
...
@pytest.mark.asyncio
async def test_candidate_evaluation_error_does_not_persist_backend_secret_text(tmp_path: Path, ):
    ...
    serialized += (Path(request.output_dir) / "runs" / request.run_id / "audit.json").read_text(encoding="utf-8")
...
async def test_terminal_journal_survives_nonessential_writeback_artifact_failure(
    ...
    journal_path = (Path(request.output_dir) / "runs" / f".{request.run_id}.tmp" / "writeback_journal.json")
    journal = json.loads(journal_path.read_text(encoding="utf-8"))
    assert (tmp_path / "prompt.txt").read_bytes() == b"safe prompt"
    assert journal["state"] == "applied"
    assert journal["after_hashes"] == {
...
@pytest.mark.asyncio
async def test_backend_neutral_core_rejects_hardlinked_train_and_validation(tmp_path: Path, ):
    request = _request(tmp_path)
...
def test_factory_rejects_hardlinked_train_and_validation_before_backend_creation(tmp_path: Path, ):
    request = _request(tmp_path)
    validation_path = Path(request.validation_path)
    validation_path.unlink()
    os.link(request.train_path, validation_path)
...
@pytest.mark.asyncio
async def test_backend_neutral_core_rejects_target_fields_for_same_resolved_file(tmp_path: Path, ):
    request = _request(tmp_path)
    prompt_path = request.target_prompt_paths["system_prompt"]
    request = replace(
        request,
...
async def test_backend_neutral_core_rejects_physical_or_casefold_target_aliases(
    ...
    request = replace(
        request,
        target_prompt_paths={
            "system_prompt": first_path,
            "router_prompt": second_path
        },
    )
    backend = _safe_backend(candidate_count=1)
...
def test_shared_path_validator_handles_missing_casefold_collisions(tmp_path: Path):
    ...
    with pytest.raises(ValueError, match="case-insensitively"):
        pipeline_module.validate_distinct_file_paths(
            {
                "first": tmp_path / "Missing.txt",
                "second": tmp_path / "missing.txt"
            },
            context="test paths",
        )
...
async def test_candidate_prompts_must_be_valid_utf8_before_evaluation(tmp_path: Path):
    request = _request(tmp_path)
    backend = _safe_backend(candidate_count=0)
    backend.candidates = [CandidatePrompt("candidate", "bad\ud800prompt", "safe", "diff")]
...
async def test_public_report_redacts_secrets_and_absolute_personal_paths(tmp_path: Path):
    ...
    assert (report.audit["config_hash"] == hashlib.sha256(
    ...
    config_snapshot = Path(request.output_dir).joinpath(
        "runs",
        request.run_id,
    ...
    assert hashlib.sha256(
        config_snapshot.encode("utf-8")).hexdigest() == report.audit["redacted_config_snapshot_sha256"]
    assert report.audit["config_file_sha256"] == hashlib.sha256(Path(
        request.optimizer_config_path).read_bytes()).hexdigest()
...
async def test_late_no_write_source_mutation_downgrades_journal_to_unknown(
    ...
    assert journal["after_hashes"] == {"system_prompt": hashlib.sha256(b"mutation after final artifacts").hexdigest()}
    assert not (Path(request.output_dir) / "runs" / request.run_id).exists()
