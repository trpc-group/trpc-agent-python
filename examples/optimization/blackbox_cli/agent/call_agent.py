...
def _build_cli_env() -> dict[str, str]:
    """把通用 TRPC_AGENT_* 三件套映射成 CLI 期望的 ANTHROPIC_* 环境变量。

    用户只需配通用三件套，无需为 CLI 单独配置 ANTHROPIC_API_KEY。
    """
    env = dict(os.environ)
    ...
    if not (api_key and model_name):
        raise RuntimeError(
            "TRPC_AGENT_API_KEY / TRPC_AGENT_MODEL_NAME "
            "must be set so they can be forwarded to claude CLI."
        )
...
async def _run_cli(query: str) -> str:
    ...
    """
    env = _build_cli_env()
    model_name = env.pop("ANTHROPIC_MODEL", None)
    cmd = [
        CLI_BINARY,
        "--print",
        "--add-dir",
    ...
    try:
        ...
        await proc.wait()
        raise RuntimeError(
            f"claude timed out after {CLI_TIMEOUT_SEC}s on query={query!r}"
        )
