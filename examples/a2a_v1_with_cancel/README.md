# A2A Agent Cancel 示例（a2a-sdk 1.x）

本示例演示如何通过 **a2a-sdk 1.x** 部署支持远程 Cancel 的 Agent 服务。
0.3 取消示例见 [examples/a2a_with_cancel](../a2a_with_cancel/README.md)。
...
- [examples/a2a_v1_with_cancel/run_server.py](./run_server.py)：A2A 服务端入口（`create_a2a_application`；配置 cancel_wait_timeout）
- [examples/a2a_v1_with_cancel/test_a2a_cancel.py](./test_a2a_cancel.py)：A2A 客户端取消测试（两个场景）
- [examples/a2a_v1_with_cancel/agent/agent.py](./agent/agent.py)：Agent 定义（LlmAgent + 天气工具）
- [examples/a2a_v1_with_cancel/agent/tools.py](./agent/tools.py)：天气查询工具（含延时与执行日志）
- [examples/a2a_v1_with_cancel/agent/config.py](./agent/config.py)：模型配置（从环境变量读取）
- [examples/a2a_v1_with_cancel/agent/prompts.py](./agent/prompts.py)：Agent 提示词
- [examples/a2a_v1_with_cancel/.env](./.env)：环境变量配置文件
...
在 [examples/a2a_v1_with_cancel/.env](./.env) 中设置（也可通过 export）：
...
```bash
cd examples/a2a_v1_with_cancel
...
| 文件 | 说明 |
|---|---|
| `run_server.py` | A2A 服务端入口（a2a-sdk 1.x，`create_a2a_application`；配置 cancel_wait_timeout） |
...
- 需要验证基本 A2A 服务（不含 Cancel）：建议使用 `examples/a2a_v1`
- a2a-sdk 0.3 取消示例：建议使用 `examples/a2a_with_cancel`
