...
"""
Weather Agent with Cancellation Demo

This example demonstrates the agent cancellation feature in trpc-agent,
...
async def scenario_1_cancel_during_streaming() -> None:
    """Scenario 1: Cancel while LLM is streaming response.
    ...
    Queries:
    1. Ask for weather report (will be cancelled after 10 events)
    2. Ask "what happens?" to verify session state
    """

    print("📋 Scenario 1: Cancel During LLM Streaming")
    print("-" * 80)

    # Common settings for this scenario
    app_name = "weather_agent_cancel_demo"
    user_id = "demo_user"
    ...
    # Query 1: Ask for introduce (will be cancelled)
...
async def run_with_cancellation_demo() -> None:
    """Demonstrate agent cancellation in realistic scenarios."""

    print("=" * 80)
    print("🎯 Agent Cancellation Demo")
    print("=" * 80)
    print()
