...
- **WebSearch工具因为Antropic的封控，经过测试是不能使用的。**
...
<p align="center">
  <img src="imgs/claude_agent_architecture.png" alt="ClaudeAgent's Architecture" />
</p>
...
```python
# agent/agent.py
from trpc_agent_ecosystem.agents.claude import ClaudeAgent, setup_claude_env, destroy_claude_env
...
运行Agent程序，示例输出如下所示，可以看到，ClaudeAgent根据用户指令自动编写代码并写入文件，大家可以替换 `demo_queries` 中的提问文本为适合自己场景的例子，更多示例也可参考[trpc-agent的examples目录](../../examples)：
```text
...
```python
from trpc_agent.runners import Runner, RunConfig
...
```python
from trpc_agent.tools import FunctionTool, MCPToolset
...
```python
from trpc_agent_ecosystem.agents.claude import SessionConfig
...
from trpc_agent.types import GenerateContentConfig
from trpc_agent.models import OpenAIModel
from trpc_agent_ecosystem.agents.claude import setup_claude_env
