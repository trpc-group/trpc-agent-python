...
async def test_session_service_persists_and_restores_events(tmp_path: Path) -> None:
    ...
    metadata = json.loads(
        (first.runtime.for_session(session).paths.session_dir(session.id) / "session.json").read_text(encoding="utf-8"))
    ...
    assert restored.state["session-key"] == "session-value"
    assert restored.state["app:theme"] == "dark"
...
async def test_same_session_id_is_isolated_between_users(tmp_path: Path) -> None:
    ...
    assert (await service.get_session(app_name="demo-app", user_id="user-a",
                                      session_id="shared-session")).events[0].id == "event-a"
    assert (await service.get_session(app_name="demo-app", user_id="user-b",
                                      session_id="shared-session")).events[0].id == "event-b"
    assert service.runtime.for_session(first).paths.session_dir(
        first.id) != service.runtime.for_session(second).paths.session_dir(second.id)
...
async def test_delete_session_removes_persistent_session_data(tmp_path: Path) -> None:
    ...
    metadata = json.loads((service.runtime.for_session(session).paths.session_dir(session.id) /
...
def test_transcript_decorator_can_cross_event_loops(tmp_path: Path) -> None:
    ...
    asyncio.run(create())
    asyncio.run(append())
    records = asyncio.run(service.runtime.for_scope("demo-app", "demo-user").transcripts.read_all("wrapped-session"))
