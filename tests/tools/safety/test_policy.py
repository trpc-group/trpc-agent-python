...
from trpc_agent_sdk.tools.safety import RegexRule
...
def test_custom_policy_changes_behaviour_without_code(tmp_path: Path) -> None:
    ...
    report = scanner.scan(ScanInput(script="echo FORBIDDEN_MARKER_TOKEN", language=ScriptLanguage.BASH))
...
def test_regex_rule_compiled_is_cached() -> None:
    """``compiled()`` returns the same pattern object across calls (no rebuild)."""
    rule = RegexRule(
        rule_id="T1",
    ...
    assert first.search("rm -rf /") is not None
