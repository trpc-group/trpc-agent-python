...
def test_resolve_model_raises_when_missing() -> None:
    ...
    with pytest.raises(ValueError, match="sub-agent: cannot resolve model"):
...
def test_build_sub_agent_filters_out_nesting_tools() -> None:
    """Neither SpawnSubAgentTool nor DynamicSubAgentTool must reach the sub-agent.
    ...
    assert "DynamicSubAgentTool" not in tool_names
