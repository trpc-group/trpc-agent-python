...
class _DuplicateFailedExecutor(AgentExecutor):
    """Enqueue Task → working → FAILED → FAILED, matching the error_code path bug."""
    ...
    async def execute(self, context, event_queue) -> None:
        ...
        await event_queue.enqueue_event(
            _status_event(task_id, context_id, TaskState.TASK_STATE_FAILED, "first failed")
        )
        await event_queue.enqueue_event(
            _status_event(task_id, context_id, TaskState.TASK_STATE_FAILED, "second failed")
        )
...
@pytest.mark.asyncio
async def test_handler_does_not_dedupe_duplicate_failed_status():
    """a2a-sdk 1.x DefaultRequestHandler forwards both terminal FAILED updates.

    The executor must therefore skip a trailing create_final_status_event(FAILED)
    when convert_event_to_a2a_events already published one.
    """
    executor = _DuplicateFailedExecutor()
    handler = DefaultRequestHandler(
        agent_executor=executor,
        task_store=InMemoryTaskStore(),
        agent_card=_agent_card(),
    ...
    assert len(failed) == 2
    assert failed[0].status.message.parts[0].text == "first failed"
    assert failed[1].status.message.parts[0].text == "second failed"
