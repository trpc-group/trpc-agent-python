...
class TestOpenAIResponsesAPI:
    ...
    def test_rejects_add_tools_to_prompt_with_responses_api(self):
        """The Responses API uses native function tools, so prompt-injected
        tool definitions would be silently ignored — combining the two must
        fail loudly at construction instead of breaking tool calling at runtime.
        """
        with pytest.raises(ValueError, match="incompatible with add_tools_to_prompt"):
            _model(use_responses_api=True, add_tools_to_prompt=True)

        # Each option on its own remains valid.
        assert _model(use_responses_api=True).use_responses_api is True
        assert _model(add_tools_to_prompt=True).add_tools_to_prompt is True
    ...
    def test_reorders_function_call_outputs_after_function_call(self):
        """Responses input must place function_call_output after its matching
        function_call; orphaned outputs from legacy message ordering (tool
        response before the assistant tool_calls message) are reordered."""
        model = _model(use_responses_api=True)
        messages = [
            # Legacy ordering: tool response precedes the assistant tool_calls
            # message, as produced for mixed single-Content inputs.
            {
                "role": "tool",
                "tool_call_id": "call_weather",
                "content": '{"temperature":30}'
        ...
        items = model._convert_messages_to_responses_input(messages)
        item_types = [item.get("type") or item.get("role") for item in items]
        assert item_types == ["user", "function_call", "function_call_output"]
        assert items[1]["call_id"] == "call_weather"
        assert items[2]["call_id"] == "call_weather"
    ...
    @pytest.mark.asyncio
    async def test_non_streaming_maps_function_call_output(self):
        """A non-streaming Responses payload with a function_call item maps to
        a LlmResponse carrying the function_call part (id/name/args)."""
        model = _model(use_responses_api=True)
        request = _request(
            [Content(parts=[Part.from_text(text="Use lookup")], role="user")],
            GenerateContentConfig(max_output_tokens=128),
        )
        response = Mock()
        response.model_dump.return_value = {
    ...
    @pytest.mark.asyncio
    async def test_responses_maps_prompt_cache_and_passthrough_parallel_tool_calls(self):
        """prompt_cache_* resolved from the model-level PromptCacheConfig are
        mapped to their Responses names; parallel_tool_calls is forwarded
        verbatim via responses_api_params."""
        ...
        model = _model(
            ...
            responses_api_params={"parallel_tool_calls": True},
        )
        request = _request(
            [Content(parts=[Part.from_text(text="Hi")], role="user")],
            GenerateContentConfig(max_output_tokens=128),
        )
        response = Mock()
        response.model_dump.return_value = {
        ...
        with patch.object(model, "_create_async_client") as client_factory:
            ...
            client.responses.create = create
            client_factory.return_value = client
            [item async for item in model.generate_async(request, stream=False)]

        assert captured.get("prompt_cache_key") == "cache-1"
        assert captured.get("prompt_cache_retention") == "1h"
        assert captured.get("parallel_tool_calls") is True

    @pytest.mark.asyncio
    async def test_streaming_multiple_function_calls_accumulate(self):
        """Parallel function_calls stream and accumulate independently, with
        each call's arguments assembled from interleaved deltas."""
        model = _model(use_responses_api=True)
        request = _request(
            [Content(parts=[Part.from_text(text="Use both tools")], role="user")],
            streaming_tool_names={"lookup_a", "lookup_b"},
        )
        events = [
            {
        ...
        # Streaming deltas for both calls were yielded, interleaved by call.
        tool_deltas = [(item.content.parts[0].function_call.name,
                        item.content.parts[0].function_call.args["tool_streaming_args"]) for item in responses[0:-1]]
        assert ("lookup_a", '{"q":') in tool_deltas
        assert ("lookup_b", '{"r":') in tool_deltas
        assert ("lookup_a", '"a"}') in tool_deltas
        assert ("lookup_b", '"b"}') in tool_deltas
        # Final response carries both fully-assembled function_calls.
        final = responses[-1]
        calls = {
            part.function_call.name: part.function_call.args
            for part in final.content.parts if part.function_call is not None
        }
        assert calls == {
            "lookup_a": {
                "q": "a"
    ...
    def test_pure_reasoning_turn_preserves_responses_input_items(self):
        """A turn with only thought parts (no text/image/tool calls) must
        still produce a message carrying ``responses_input_items`` so that
        reasoning signatures are not silently dropped from history.
        """
        from trpc_agent_sdk.models._openai_model import _RESPONSES_INPUT_ITEMS
        model = _model(use_responses_api=True)

        reasoning_item = {
            "type": "reasoning",
            "id": "rs_pure",
            "encrypted_content": "encrypted-pure",
            "summary": [{
                "type": "summary_text",
                "text": "Pure thought"
            }],
        ...
        # Assistant content with only a thought part — no text, no tool calls.
        # thought_signature is set via attribute assignment (not construction)
        # to bypass Part's base64 validation, matching _create_responses_response.
        thought_part = Part(thought=True, text="")
        thought_part.thought_signature = json.dumps(reasoning_item, ensure_ascii=False).encode("utf-8")
        assistant_content = Content(
            role="model",
            parts=[thought_part],
        )
        user_content = Content(role="user", parts=[Part.from_text(text="follow up")])

        messages = model._format_messages(_request([assistant_content, user_content]))

        # The assistant message must be present and carry the reasoning items.
        assistant_msgs = [m for m in messages if m.get("role") == "assistant"]
        assert len(assistant_msgs) == 1
        assert _RESPONSES_INPUT_ITEMS in assistant_msgs[0]
        assert assistant_msgs[0][_RESPONSES_INPUT_ITEMS] == [reasoning_item]

    @pytest.mark.asyncio
    async def test_responses_thinking_budget_does_not_map_to_effort(self):
        """A positive thinking_budget must not be mapped to reasoning.effort:
        supported effort values vary by model and are passed through verbatim
        via ``responses_api_params`` instead."""
        from trpc_agent_sdk.types import ThinkingConfig

        model = _model(use_responses_api=True)
        request = _request(
            [Content(parts=[Part.from_text(text="Think hard")], role="user")],
            GenerateContentConfig(
    ...
    @pytest.mark.asyncio
    async def test_responses_thinking_budget_negative_one_skips_effort(self):
        """thinking_budget=-1 (automatic) should not set reasoning.effort,
        letting the model decide."""
        from trpc_agent_sdk.types import ThinkingConfig

        model = _model(use_responses_api=True)
        request = _request(
            [Content(parts=[Part.from_text(text="Think")], role="user")],
            GenerateContentConfig(
                max_output_tokens=4096,
                thinking_config=ThinkingConfig(
                    include_thoughts=True,
    ...
    @pytest.mark.asyncio
    async def test_responses_drops_chat_completions_only_params(self):
        """Params with no Responses equivalent (stop, penalties, seed, n) must
        not be forwarded, otherwise the OpenAI API rejects the request."""
        model = _model(use_responses_api=True)
        request = _request(
            [Content(parts=[Part.from_text(text="Hi")], role="user")],
            GenerateContentConfig(
        ...
        # Supported framework params are forwarded with their Responses name.
        assert captured.get("temperature") == 0.7
        # Chat Completions-only params are dropped, not forwarded.
        for dropped in ("stop", "frequency_penalty", "presence_penalty", "seed", "n"):
            assert dropped not in captured

    @pytest.mark.asyncio
    async def test_responses_api_params_passthrough_reasoning_effort(self):
        """reasoning.effort is passed through verbatim from responses_api_params,
        and a user-supplied summary wins over the framework default."""
        from trpc_agent_sdk.types import ThinkingConfig
        ...
        assert captured.get("reasoning") == {"effort": "xhigh", "summary": "detailed"}
